...
static void testUseIPCachePersistentClients(void)
{
    ...
    ret = asRefreshHag(&changed);
    testOk(!asCheckGet(oldClient) && !asCheckPut(oldClient),
        "original-address client loses access after explicit HAG refresh");
    testOk(asCheckGet(newClient) && !asCheckPut(newClient),
        "later-address client gains read-only access after explicit HAG refresh");
    testOk(ret==0 && changed==1 && oldCallbackCount==2 && newCallbackCount==2,
        "explicit refresh reports change and callbacks fire once per client transition");
